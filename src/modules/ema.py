...
# Adapted for easy usage with Pytorch Lightning
from collections import OrderedDict
from typing import IO, Any, Optional, Self, Union
...
class ExponentialMovingAverage:
    ...
    def update(self, model: torch.nn.Module) -> None:
        """Updates the stored parameters using the state dict of the provided module.

        The module should have the same structure as that used to initialize the
    ...
    def load_state_dict(self, state_dict: OrderedDict) -> None:
        for k in state_dict["params"].keys():
            self.params[k] = state_dict["params"][k].clone().to(self.device)
...
class EMACheckpointMixin:
    """Mixin for loading models with EMA weights."""
    ...
    def load_from_checkpoint(
        ...
        **kwargs: Any,
        ...
        model = super().load_from_checkpoint(
            checkpoint_path, map_location, hparams_file, strict=strict, **kwargs
        )

        if use_ema:
            checkpoint = torch.load(checkpoint_path, map_location=map_location)
            if "ema" in checkpoint and "params" in checkpoint["ema"]:
                print("Loading EMA weights")
                model.load_state_dict(checkpoint["ema"]["params"], strict=strict)
            else:
